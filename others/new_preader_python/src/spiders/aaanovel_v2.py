...
class AaanovelParser(BaseParser):
    """aaanovel.com 小说解析器 - 配置驱动版本"""
    ...
    # 基本信息
    name = "aaanovel.com"
    description = "aaanovel.com 小说解析器"
    base_url = "https://aaanovel.com"
    
    # 正则表达式配置
    title_reg = [
        r"<h1 class='entry-title' itemprop='headline'>(.*?)</h1>",
        r'<title>(.*?)</title>'
    ]
    
    content_reg = [
        r"<div class='entry-content' itemprop='text'>(.*?)</div>"
    ]
    ...
    after_crawler_func = [
        ...
        "_remove_ads",  # 广告移除
        "_clean_aaanovel_content"  # aaanovel.com特定内容清理
    ]
    ...
    def get_novel_url(self, novel_id: str) -> str:
        """
        重写URL生成方法，适配aaanovel.com的URL格式
    ...
    def _clean_aaanovel_content(self, content: str) -> str:
        """
        aaanovel.com特定内容清理 - 移除指定文本
