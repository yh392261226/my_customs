...
class Rouwenwu20Parser(BaseParser):
    """rouwenwu20.com 小说解析器"""
    # 网站使用GBK编码
    encoding = "gbk"
    # 基本信息
    name = "rouwenwu20.com"
    description = "rouwenwu20.com 小说解析器"
    base_url = "https://www.rouwenwu20.com"
    ...
    desc_reg = [
        r'<div[^>]*class="describe-html"[^>]*>(.*?)</div>'
    ]
    
    tags_reg = [
        r'<div[^>]*class="book-describe"[^>]*>(.*?)</div>'
    ]
    ...
    def get_novel_url(self, novel_id: str) -> str:
        """
        根据小说ID生成小说URL
        格式: https://www.rouwenwu20.com/{prefix}_{novel_id}/
        其中 prefix 是小说ID去掉后3位后剩余的数值，如果不足1则显示为0
        ...
        if len(novel_id) > 3:
            prefix = novel_id[:-3]
            if not prefix:  # 如果为空字符串
                prefix = "0"
        else:
            prefix = "0"
            
        return f"{self.base_url}/{prefix}_{novel_id}/"
    
    def _detect_book_type(self, content: str) -> str:
        """
        检测书籍类型
        rouwenwu20.com 主要是多章节小说
    ...
    def _parse_multichapter_novel(self, content: str, novel_url: str, title: str) -> Dict[str, Any]:
        ...
        if tags_match:
            # 从book-describe div中提取所有p标签内容
            p_tags = re.findall(r'<p[^>]*>(.*?)</p>', tags_match, re.DOTALL)
            if p_tags:
                # 清理每个p标签内容并用逗号连接
                tags = [self._clean_html_text(tag).strip() for tag in p_tags if self._clean_html_text(tag).strip()]
    ...
    def _extract_chapter_links(self, content: str) -> List[Dict[str, str]]:
        ...
        # 查找章节列表区域
        chapter_list_match = re.search(r'<div[^>]*class="book-list clearfix"[^>]*>.*?</div>', content, re.DOTALL)
        ...
        # 提取所有章节链接
        chapter_matches = re.findall(r'<a[^>]*target="_blank"[^>]*title="([^"]*)"[^>]*href="([^"]*)"[^>]*>(.*?)</a>', chapter_list_html)
    ...
    def _get_chapter_content(self, chapter_url: str) -> Optional[str]:
        ...
        try:
            ...
            # 提取章节内容 - 使用id="nr1"的div
            content_match = re.search(r'<div[^>]*id="nr1"[^>]*>(.*?)</div>', chapter_html, re.DOTALL)
