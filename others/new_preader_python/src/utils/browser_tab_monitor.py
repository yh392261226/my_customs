...
class BrowserTabMonitor:
    """浏览器标签页监控器（支持Safari、Chrome、Brave、Firefox）"""

    def __init__(self, novel_sites=None, on_url_detected=None, headless=True, browser_type=None):
        """
        初始化浏览器标签页监控器
    ...
    def close_tab(self, url: str) -> bool:
        ...
        try:
            ...
            if self.browser_type == BrowserType.SAFARI:
                ...
                # Firefox不支持AppleScript
                logger.warning("Firefox暂不支持关闭标签页")
                return False
            else:  # Chrome
                app_name = "Google Chrome"
            ...
            if result.returncode == 0:
                ...
                if output == "closed":
                    ...
                    logger.warning(f"{app_name}未运行，无法关闭标签页")
                    return False
                else:
