...
function fzf_brew_install_or_remove
    # Desc: function: fzf_brew_install_or_remove: Fully manage brew installation and suppression, and then some.
    ...
    set -l reload "reload%switch (cat $tmp_file)
        case install
            echo Install mode
            brew tap-info --json --installed | jq --raw-output '$jq_all' | sort
        case '*'
            echo Remove mode
            brew info --json=v2 --installed | jq --raw-output '$jq_installed' | sort
        end"
    ...
    set -l c "\e[1;36m"
    set -l d "\e[1;37m"
    set -l help "$bold$c[B$d]{$d}rew $c[I$d]{$d}nteractive$reset
